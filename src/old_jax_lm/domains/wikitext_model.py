...
class SelfAttention(nn.Module):
    ...
    def __call__(self, x, mask, deterministic=None):
        ...
        qkv = nn.Dense(3 * C, use_bias=self.use_proj_bias, dtype=self.dtype,
                       name='c_attn')(x)
...
class GPT(nn.Module):
    ...
    def __call__(self, idx, deterministic=None):
        ...
        x = nn.LayerNorm(1e-6, dtype=self.config.dtype, use_bias=self.config.use_bias, name='ln_f')(x)
        logits = wte.attend(x)
        return logits
...
def convert_hf_params(hf_params: FrozenDict, num_heads, num_embeds,
                      dtype=jnp.float32) -> FrozenDict:
    ...
    params = unflatten_dict({f'params.{k}': v.astype(dtype) for k, v in params.items()}, sep='.')
    # cast to float32
    return freeze(params)

def get_pretrained_model(model_type):
    config, params = get_pretrained_params(model_type)
    model = GPT(config)
    return model, params
...
def get_pretrained_params(model_type: str) -> Tuple[GPTConfig, FrozenDict]:
    ...
    config = {
        'gpt2':        GPTConfig(num_layers=12, num_heads=12, dtype=jnp.float32, num_embeds=768),  # 124M params
        'gpt2-medium': GPTConfig(num_layers=24, num_heads=16, dtype=jnp.float32, num_embeds=1024), # 350M params
        'gpt2-large':  GPTConfig(num_layers=36, num_heads=20, dtype=jnp.float32, num_embeds=1280), # 774M params
        'gpt2-xl':     GPTConfig(num_layers=48, num_heads=25, dtype=jnp.float32, num_embeds=1600), # 1558M params
    }[model_type]
