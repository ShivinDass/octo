...
def make_cfgs():
    base_cfg = cfg
    b1_b2s = [(0.95, 0.975)]
    eps0_epss = [(10**(-k), 1e-13) for k in range(7, 11)] + [(10**(-k), 1e-15) for k in range(7, 11)] 
    eps_stepss = [100, 200]
    mom0s = [1]
    lora_stds = [1e2]
    learning_rates = [0.0002, 0.000275, 0.00035]

    cfgs = []
    this_product = product(b1_b2s, eps0_epss, mom0s, lora_stds, learning_rates, eps_stepss)
    for (b1, b2), (eps0, eps), mom0, lora_std, lr, eps_steps in this_product:
        update = {
            'train_frac': 0.4,
            'b1': b1,
            'b2': b2,
