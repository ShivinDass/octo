...
def main(seed=0):
    ...
    lr_mapping = jax.tree_util.tree_map_with_path(param_labels, model_state['params'])
    tx = optax.multi_transform(
        {'norm': optax.chain(
                optax.add_decayed_weights(wd / lr_biases),
                optax.sgd(learning_rate=jax.tree_util.Partial(get_lr, base_lr=lr_biases, total_train_steps=total_train_steps), 
                          momentum=MOMENTUM, 
                          nesterov=True)),
         'other': optax.chain(optax.add_decayed_weights(wd / lr),
                optax.sgd(learning_rate=jax.tree_util.Partial(get_lr, base_lr=lr, total_train_steps=total_train_steps), 
                          momentum=MOMENTUM, 
                          nesterov=True)),
         'Conv_0': optax.set_to_zero()}, param_labels=lr_mapping)

    opt_state = tx.init(model_state['params'])

    def loss_fn(state, inputs, labels):
        labels = jax.nn.softmax(labels)
        logits, new_vars = model.apply(state, inputs, training=True, mutable=['batch_stats'])
        new_batch_stats = new_vars['batch_stats']
        per_example_loss = optax.softmax_cross_entropy(logits, labels)
        return jnp.sum(per_example_loss), (logits, new_batch_stats)

    @jax.jit
    def eval_step(state, images, labels):
        logits = model.apply(state, images, tta_level=TTA_LEVEL, training=False)
        preds = jnp.argmax(logits, axis=1)
        correct = jnp.sum(preds == jnp.argmax(labels, axis=1))
        total = labels.shape[0]
        return correct, total

    def evaluate(state, val_fn, val_num_its):
        correct = 0
        total = 0
        for i in range(val_num_its):
            idx, (images, labels, _) = val_fn(i)
            correct, total = eval_step(state, images, labels)
            correct += correct
            total += total
        return (correct / total).item()

    @jax.jit
    def train_step(state, opt_state, inputs, labels, step):
        gradder = jax.value_and_grad(loss_fn, has_aux=True)
        (loss, (logits, new_batch_stats)), grads = gradder(state, inputs, labels)
        updates, opt_state = tx.update(grads['params'], opt_state, state['params'])
        new_params = optax.apply_updates(state['params'], updates)
        return {'params': new_params, 'batch_stats': new_batch_stats}, opt_state, loss, logits
