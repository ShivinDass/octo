...
def backward_step(batch, state, next_state, dstate, vjp_skele,
    ...
    # PREV HAD THIS
    # deps2, dstate2 = jax.tree.map(partial(safe_divide, y=count), (deps2, dstate2))
    # deps2, dstate2 = deps2, d
    # deps = jax.tree.map(safe_add, deps, deps2)
    # dstate = jax.tree.map(safe_add, dstate, dstate2)
...
def factored_functional_step(*, use_jit=True, lr_factor=get_one(),
    ...
    def get(state, batch, per_sample_loss):
        ...
        if use_jit:
            ...
            # grads_acc = jax.tree.map(partial(safe_divide, y=batchl_+),
            #                          grads_acc)
        else:
