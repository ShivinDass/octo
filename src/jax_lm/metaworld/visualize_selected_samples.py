...
def plot_percentages(filter_info, all_info, title, output_dir, task_name=None):
    ...
    custom_legend_colors = [
        ...
        mpatches.Patch(color='r', label='Successful Traj (Heuristic Tasks)'),
        mpatches.Patch(color='b', label='Failed Traj')
    ]
    ax.legend(handles=custom_legend_colors, loc='upper right')
    # plt.show()
    plt.savefig(f'{output_dir}/percentage_{title}.png', bbox_inches='tight')
...
if __name__ == "__main__":
    ...
    all_files = sorted(glob.glob(os.path.join(data_folder, "**", "*.h5"), recursive=True))
    
    # selected_mask, count_dms = load_dm_mask(dm_path, topk=topk, iter_num=10)
    # work(f"{count_dms}", selected_mask, all_files)

    selected_mask, count_dms = load_dm_mask(dm_path, topk=topk, iter_num=None)
    work(f"{count_dms}", selected_mask, all_files)
