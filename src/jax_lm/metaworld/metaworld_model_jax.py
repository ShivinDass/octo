...
# def create_mw_policy_2(seed, obs_dim=39):
#     return TanhGaussianBCPolicy2(
#         obs_dim=obs_dim,
#         act_dim=4,
#         hidden_sizes=[400, 400, 400],
#         init_std=1.0,
...
# def create_train_state(seed, lr=3e-4):
#     from metaworld_model_jax import create_mw_policy_2
#     policy = create_mw_policy_2(seed=seed, obs_dim=39)
#     rng = jax.random.PRNGKey(seed)
#     dummy_obs = jnp.zeros((1, 39))
#     variables = policy.init(rng, dummy_obs)
#     params = variables['params']

#     tx = optax.adamw(learning_rate=lr)
#     return policy, train_state.TrainState.create(apply_fn=policy.apply, params=params, tx=tx)

# @jax.jit
# def train_step(state, obs_batch: jnp.ndarray, act_batch: jnp.ndarray, reduce: bool = True):
#     def loss_fn(params):
#         actions_clipped = jnp.clip(act_batch, -1 + 1e-6, 1 - 1e-6)
#         # pre_tanh_value = 
#         dist = state.apply_fn({'params': params}, obs_batch)
#         log_prob = dist.log_prob(actions_clipped)
#         loss = -log_prob
#         return loss.mean() if reduce else loss, loss
...
# class TanhGaussianBCPolicy2(nn.Module):
#     obs_dim: int
#     act_dim: int
#     hidden_sizes: Sequence[int]
#     init_std: float = 1.0
...
#     def setup(self):
#         self.model = GaussianMLPTwoHeaded(
...
#             layer_norm=self.layer_normalization,
#         )

#     def __call__(self, obs):
#         return self.model(obs)

#     def loss(self, params: FrozenDict, obs: jnp.ndarray, actions: jnp.ndarray, reduce: bool = True) -> jnp.ndarray:
#         actions_clipped = jnp.clip(actions, -1 + self.clip_epsilon, 1 - self.clip_epsilon)
#         dist = self.apply({'params': params}, obs)
#         log_prob = dist.log_prob(actions_clipped)
#         loss = -log_prob
#         return loss.mean() if reduce else loss

#     def get_action(self, params: FrozenDict, obs: jnp.ndarray) -> jnp.ndarray:
#         dist = self.apply({'params': params}, obs[None, :])
#         return jnp.squeeze(jnp.tanh(dist.sample()))
