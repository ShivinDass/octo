...
class MetaworldDataset(Dataset):
    ...
    def __init__(self, data_folder):
        ...
        # self.hdf5_files = np.random.choice(self.hdf5_files, int(len(self.hdf5_files)*0.1), replace=False)
...
if __name__ == "__main__":
    ...
    dataloader = DataLoader(dataset, batch_size=16364, shuffle=True)

    model = make_mw_model(seed=0, obs_dim=39)
    ...
    for epoch in tqdm.tqdm(range(n_epochs)):
        ...
        for batch in dataloader:
            ...
            grads, loss = _grads_and_loss_for_batch(b, state, psl)
